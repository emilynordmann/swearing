...
This is the website to accompany the Psychology of Swearing talk held on Friday 30th August as part of the University of Glasgow Short Course series. The slides presented in the talk can be downloaded by [clicking this link]("slide.pdf").

## Reading list

If you would like more information abotu anything presented in this talk, I reccomend the following sources:
...
### One-way ANOVA, scores by gender
...
dat_swears%>%
  ...
  top_n(10) %>% 
  ungroup() %>%
  mutate(words = reorder_within(words, n, gender))%>%
  ggplot(aes(x = words, y = n, fill = words))+
  geom_col(show.legend = FALSE)+
  coord_flip() +
  scale_fill_viridis_d(option = "E") +
